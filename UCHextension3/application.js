...
var minPriceGap;
var t;
var timeDelayClickBuySell;

  if (urlSite.indexOf('gogo') > -1) {
        fuckBgogo()
        window.setInterval(kickBgogo, 6 * 1000);
  }

  function kickBgogo(){
       cancelOrder()
       var timeToSleep = getRandomArbitrary(100,2200);
  ...
  function fuckBgogo(){
    //    get market price
        ...
        if(marketPriceSpan != null){
            ...
            var ask1Price = parseFloat(document.getElementById("ask-1-price").innerHTML)
            var bid1Price = parseFloat(document.getElementById("bid-1-price").innerHTML)
            console.log("ask1Price:"+ ask1Price + "  bid1Price:"+ bid1Price)
            if(sellPriceInput!= null && buyPriceInput != null && sellAmountInput!= null && buyAmountInput!= null){
                 if(urlSite.indexOf("usdt")>-1){
//                    console.log("USDT")
                    amountMultiplier = 100000 // eth/usdt = 100000
...
//                    console.log("ETHBTC")
                    ...
                    minPriceGap = 0.000001
                 }
                 var priceGap = Math.round(priceMultiplier * parseFloat(ask1Price - bid1Price))/priceMultiplier
                 var tradePrice = Math.round(priceMultiplier * (ask1Price + bid1Price))/(2*priceMultiplier)//dung trong truong hop gap > minPriceGap
                 if(minPriceGap == priceGap){
                       sellPriceInput.value = bid1Price // marketPrice;
                       buyPriceInput.value =  ask1Price
                       timeDelayClickBuySell = 100
                       console.log("match minPriceGap: "+ minPriceGap + " vs "+ parseFloat(ask1Price - bid1Price))
                 }else {
                    tradePrice = Math.round(tradePrice * priceMultiplier)/priceMultiplier
                    console.log("not match minPriceGap -> tradePrice:" + tradePrice+ " minPriceGap:"+ minPriceGap + " vs "+ parseFloat(ask1Price - bid1Price))
                    sellPriceInput.value = tradePrice // marketPrice;
                    buyPriceInput.value =  tradePrice//marketPrice;
                    timeDelayClickBuySell = 200
                }
...
//                }
                setTimeout(clickBuy, timeDelayClickBuySell)
